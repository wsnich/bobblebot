...
function scribe.Run()
    ...
    if not ok then
        printf('\aybobblebot:\axCan\'t scribe right now (%s).', why)
        return 0
    end
    _running = true
    ...
    if scribedCount > 0 then
        printf('\aybobblebot:\axScribed %d spell(s).', scribedCount)
    else
        printf('\aybobblebot:\axNo new scrolls to scribe.')
    end
    ...
    if okU and spellupgrade then
        ...
        if spellupgrade.count() > 0 then
            printf('\aybobblebot:\ax%d spell upgrade(s) available -- Status tab or /cz upgrades', spellupgrade.count())
        end
    end
    return scribedCount
end
