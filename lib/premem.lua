...
local function buildUniqueGemMap()
    ...
    local function consider(gem, spell)
        if type(gem) ~= 'number' or not spell or spell == '' then return end
    ...
    for _, section in ipairs(SECTIONS) do
        ...
        for i = 1, n do
            ...
            if entry then consider(entry.gem, entry.spell) end
        end
    end
    -- Pull spell is stored differently (a single table, not a .spells array).
    local pull = botconfig.config.pull
    if pull and type(pull.spell) == 'table' then consider(pull.spell.gem, pull.spell.spell) end
